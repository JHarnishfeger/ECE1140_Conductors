...
QString trainUI::setBFail(){
  ...
  if (mainTrain->getBrakeFail() == true){
    return "FAIL";
  }
  else{
    return "No Failures";
...
QString trainUI::setEFail(){
  if (mainTrain->getEngineFail() == true){
    return "FAIL";
  }
  else{
    return "No Failures";
...
QString trainUI::setSFail(){
  if (mainTrain->getSignalFail() == true){
    return "FAIL";
  }
  else{
    return "No Failures";
...
{
    qDebug() << "Engine Fail(button clicked start): " << mainTrain->getEngineFail();
    if (mainTrain->getEngineFail() == true){
        mainTrain->setEngineFail(0);
    }
    else {
        mainTrain->setEngineFail(1);
    }
    qDebug() << "Engine Fail(button clicked end): " << mainTrain->getEngineFail();
...
{
    if (mainTrain->getBrakeFail() == true){
        mainTrain->setBrakeFail(0);
    }
    else {
        mainTrain->setBrakeFail(1);
    }
...
{
    if (mainTrain->getSignalFail() == true){
        mainTrain->setSignalFail(0);
    }
    else {
        mainTrain->setSignalFail(1);
    }
...
void trainUI::on_pushButton_clicked()
{
    if (mainTrain->getBrakeFail() == false){
        if(!mainTrain->getEbrakes()){
            ...
            mainTrain->setEngineFail(true);
        }
        else{
            //ui->progressBar_2->setValue(25);
            mainTrain->setEngineFail(false);
        }
    }
}
...
void trainUI::callTCData(uint8_t currentBlock){
    qDebug() << "current block: " << currentBlock;
