...
void trainMaker::BeaconData(uint16_t BeaconData,int ID){
    for(unsigned long long int i=0;i<SoftwareTrains.size();i++){
        ...
        else if(HardwareTrains[i]->mainTrain->getID() == ID){
            HardwareTrains[i]->mainTrain->setBeaconData(BeaconData);
        }
    }
}
...
void trainMaker::setAuth(double authDistance,double authSpeed,int ID){
    for(unsigned long long int i=0;i<SoftwareTrains.size();i++){
        if(SoftwareTrains[i]->mainTrain->getID() == ID){
            SoftwareTrains[i]->mainTrain->setAuthority(authDistance,authSpeed);
        }
        else if(HardwareTrains[i]->mainTrain->getID() == ID){
            HardwareTrains[i]->mainTrain->setAuthority(authDistance,authSpeed);
        }
    }
}
